...
5.  **Coordination via State**: Agents do not "chat" directly. They coordinate solely by reading/writing shared state in the **Beads** database (e.g., Blockers, comments, status updates).
6.  **Self-Evident UX**: The UI should require no documentation and no explanation. If users need help text to understand the interface, the design has failed. If users need documentation to get started, the UX has failed.
...
> **Note**: The Orchestrator also monitors for repeated "Command rejected because it could not be parsed safely" errors, which indicate an agent has lost the ability to speak the tool protocol. This counts as an implicit Emergency Signal.
